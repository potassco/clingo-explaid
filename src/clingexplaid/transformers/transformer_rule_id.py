...
class RuleIDTransformer(_ast.Transformer):
    """
    Transformer that takes all the rules of a program and adds an atom with `self.rule_id_signature` in their bodys.

    This is to make the original rule the generated them identifiable even after grounding. Additionally, a choice rule
    ...
    def visit_Rule(self, node: clingo.ast.AST) -> clingo.ast.AST:  # pylint: disable=C0103
        """Add a rule_id_signature(id) atom to the body of every rule that is visited."""
    ...
    def get_assumptions(self, n_rules: int | None = None) -> set[tuple[clingo.Symbol, bool]]:
        """
        Get the rule_id_signature assumptions.

        This can be modified, depending on the number of rules contained in the transformed
