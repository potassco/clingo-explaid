...
import clingo
from clingo.ast import AST, ASTType, Function, Location, SymbolicTerm, Transformer, parse_files, parse_string
...
class ConstraintTransformer(Transformer):
    ...
    def __init__(self, constraint_head_symbol: str, include_id: bool = False):
        self._constraint_head_symbol: str = constraint_head_symbol
        self._include_id: bool = include_id
        self._constraint_id: int = 1

        self.constraint_location_lookup: dict[int, Location] = {}
    ...
    def visit_Rule(self, node: AST) -> AST:  # pylint: disable=C0103
        ...
        arguments = []
        if self._include_id:
            arguments = [SymbolicTerm(node.location, clingo.parse_term(str(self._constraint_id)))]
    ...
    def parse_files(self, paths: Sequence[str | Path]) -> str:
        ...
        """
        out: list[str] = []
        parse_files([str(p) for p in paths], lambda stm: out.append(str(self(stm))))
        return "\n".join(out)
